...
function EditLink({
  ...
  function getExpiresAt() {
    ...
    let base;
    if (!link.expiresAt) base = dayjs().utc();
    else base = dayjs(link.expiresAt).utc();

    switch (expiry) {
      case "1 hour":
        return base.add(1, "hour").toISOString();
      case "1 day":
      ...
      case "1 year":
        return base.add(1, "year").toISOString();
      default:
        return null;
    }
  }
  ...
  async function handleSubmit() {
    ...
    try {
      setLoading(true);
      const res = await fetch(`/api/file/edit-link/${link.publicId}`, {
        method: "POST",
        credentials: "include",
        headers: {
    ...
    >
      ...
      <div className="create-link-option-action-div">
        <div className="create-link-expiry-div">
          <div className="create-link-subheading">
            <Clock size={12} />
            &nbsp;{link.expiresAt ? "EXTEND EXPIRY BY" : "EXPIRES IN"}
          </div>
      ...
      <div className="create-link-password-div">
        ...
        <input
          ...
          name="password"
          placeholder={
            link.password ? "Password hidden for security" : "example: xyz"
          }
          value={password}
